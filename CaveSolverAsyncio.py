...
def drawInitial(window, width, grid):
...
# Create the GUI
def drawWindow(window, width, grid):
    for x in range(0, width, BLOCK_SIZE):
        for y in range(0, width, BLOCK_SIZE):
            row = x // BLOCK_SIZE
            col = y // BLOCK_SIZE
...
async def solveCave(window, grid, n, stack):
    while gridSolved(grid, n) == False:
        ...
        if i != -1:
            ...
            drawWindow(window, n*BLOCK_SIZE, grid)
...
async def main():
    path = "tests/" + str(int(time.time()) % 9 + 1)
    file = open(path, "r")
    ...
    if checkAllWalls(grid, n) == False:
        ...
        drawWindow(window, width, grid)

    if await solveCave(window, grid, n, stack):
